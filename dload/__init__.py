...
def save_multi(url_list, dir="", max_threads=1, tsleep=0.05):
    ...
    try:
        ...
        for url in url_list:
            ...
            dload_threads = [x.getName()
                             for x in threading.enumerate() if "dload" == x.getName()]
            while len(dload_threads) >= max_threads:
                dload_threads = [
                    x.getName() for x in threading.enumerate() if "dload" == x.getName()]
                sleep(tsleep)
    ...
    with io.BytesIO() as f:
        ...
        if total_length is None:  # no content length header
            ...
            for chunk in r.iter_content(1024):
                ...
                sys.stdout.write("\r[%s%s] %s Mbps" % (
                    '=' * done, ' ' * (30-done), dl//(time.clock() - start) / 100000))
...
def save_unzip(zip_url, extract_path="", delete_after=False):
    ...
    try:
        ...
        extract_path = extract_path if extract_path.strip() else c_path + \
            os.path.sep+folder
...
def git_clone(git_url, clone_dir=""):
    ...
    if not git_url.lower().endswith(".git"):
        ...
        return ""
    try:
        repo_name = re.sub("\.git$", "", git_url, 0,
